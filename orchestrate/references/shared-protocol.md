...
- **Model and reasoning-effort selection are mandatory and explicit, not a default.** Every dispatch call (`spawn_agent` or its v2 equivalent) MUST pass an explicit `model` AND an explicit `reasoning_effort`. Omitting either does not mean "cheapest/lightest available" — it means the sub-agent inherits the coordinator's current model and effort, silently defeating tiering. Before every dispatch, state the unit type and the chosen model tier and effort level in the same message or log entry, then set both arguments to match:
  ...
  - **Cross-family review is mandatory, not just cross-agent.** If the doer's model is from one family (e.g. an OpenAI model), the independent reviewer of that unit MUST be from a different family (e.g. an Anthropic model), and vice versa. A same-family reviewer is not a valid independent judgment for this protocol, even if it is a different model or a fresh agent — same-family models share training-data and RLHF blind spots, so the diversity of opinion the review step exists for is lost. When assigning the reviewer's `model`, check the doer's family first and deliberately pick the other one.
